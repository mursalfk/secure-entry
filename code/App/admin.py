...
from database import get_user_by_id, add_user, get_all_users, delete_user
...
def add_new_user():
    ...
    if image_file:
        ...
        add_user(name, image_data)  # This function is in `database.py`
...
def remove_user(user_id):
    ...
    delete_user(user_id)  # Calls function from database.py
    flash("User deleted successfully!", "success")
    
    return redirect(url_for('admin.dashboard'))

### --- CAPTURE FACE POSES (Now with AJAX Updates) --- ###
@admin_blueprint.route('/capture_faces/<int:user_id>', methods=['POST'])
def capture_faces(user_id):
    ...
    username = user[1]  # Get username from database
    ...
    return jsonify({"status": "success", "updates": status_updates})
